...
void ScaleAffector::affect(Particle& particle, sf::Time dt)
{
	...
	//particle.scale += sf::Vector2f(particle.scale.x * mScaleFactor.x, particle.scale.y * mScaleFactor.y) * dt.asSeconds();
	particle.scale.x = std::max(0.f, particle.scale.x);
	particle.scale.y = std::max(0.f, particle.scale.y);
}
