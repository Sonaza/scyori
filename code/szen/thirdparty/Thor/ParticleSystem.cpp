...
{
	mTexture.reset(new sf::Texture(*texture));
}
...
void ParticleSystem::emit(const size_t amount)
{
	// Emit new particles and remove expiring emitters
	for (EmitterContainer::iterator itr = mEmitters.begin(); itr != mEmitters.end(); ++itr)
	{
		itr->first->emit(*this, amount);
	}
}
